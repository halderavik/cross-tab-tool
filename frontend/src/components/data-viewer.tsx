...
export function DataViewer({ isOpen, onOpenChange }: DataViewerProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogPortal>
        <DialogContent className={`${isFullscreen ? 'max-w-[100vw] max-h-[100vh] p-2' : 'max-w-4xl max-h-[80vh]'} fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-50 bg-background`}> 
          <DialogHeader>
                  ...
                  <table className="min-w-max w-full text-sm border-collapse">
                    ...
                    <tbody>
                      {variables.map((variable) => (
                        <tr key={variable.id}>
                          <td className="px-2 py-1 border-b">{variable.name}</td>
                          <td className="px-2 py-1 border-b">{variable.label}</td>
                          <td className="px-2 py-1 border-b">{variable.type}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
